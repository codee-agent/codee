...
- 优化了Agent的多工具调用；
- 支持了Pencil工具的本地调用；
- 支持了Codee Skills的能力；修复了GLM-4.7模型在交叉思考的过程中，出现推理不连贯的情况；
- 添加来本地的qdrant向量数据库；agent中添加了codebase indexing；修复了其它已知的BUG；
- 优化了memory bank的业务逻辑；修复了其它已知的BUG；
- 添加了任务链列表的Agent功能；添加上下文管理的功能，部分模型进行上下文压缩；
