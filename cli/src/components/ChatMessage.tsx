...
/**
 * Render a tool call in webview style: "Codee wants to read this file:" / "Codee read this file:"
 */
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, mode, isStreaming }) => {
	...
	if ((type === "ask" && ask === "command") || say === "command") {
		...
		const label = isAsk ? "Codee wants to execute this command: " : "Codee executed this command: "
	...
	if ((type === "ask" && ask === "use_mcp_server") || say === "use_mcp_server") {
		...
		const actionLabel = isAsk ? "Codee wants to use MCP" : "Codee used MCP"
		const targetLine =
