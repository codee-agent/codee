...
import { CodebaseSearchHandler } from "./tools/handlers/CodebaseSearchHandler"
...
export class ToolExecutor {
	...
	private registerToolHandlers(): void {
		...
		this.coordinator.register(new CodebaseSearchHandler())
	}

	/**
	 * Main entry point for tool execution - called by Task class
	 */
	public async executeTool(block: ToolUse, useNativeToolCalls: boolean = false): Promise<void> {
		await this.execute(block, useNativeToolCalls)
	}
	 ...
	 */
	private pushToolResult = (content: ToolResponse, block: ToolUse, useNativeToolCalls: boolean = false) => {
	 ...
	 */
	private async execute(block: ToolUse, useNativeToolCalls: boolean = false): Promise<boolean> {
		...
		try {
			...
			await this.handleCompleteBlock(block, config, useNativeToolCalls)
			return true
		} catch (error) {
	 ...
	 * @param config The task configuration containing all necessary context
	 */
	private async handleCompleteBlock(block: ToolUse, config: any, useNativeToolCalls: boolean = false): Promise<void> {
		...
		try {
			...
			this.pushToolResult(toolResult, block, useNativeToolCalls)
