...
export class Task {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const codeIndexManager = await CodeIndexManager.getInstance(this.controller.context, this.cwd) 
		...
		// 	// 写入到本地文件
		// 	await fs.appendFile('/Users/xxxx/api_log.txt', `\n[@@@@@@@@@@@@@@ request]\n${logContent}\n\n\n[############### response]\n\n`);
		// 	// 控制台输出
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				await this.toolExecutor.executeTool(block, this.useNativeToolCalls)
				if (block.call_id) {
	...
	async recursivelyMakeClineRequests(userContent: ClineContent[], includeFileDetails: boolean = false): Promise<boolean> {
		...
		if (partialBlocks.length > 0 || partialToolBlocks?.length > 0) {//huqb
			...
			if (assistantHasContent) {
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that Codee cannot process. Retrying the request may help resolve this issue."
