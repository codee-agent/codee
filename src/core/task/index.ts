...
import {
	...
	SYSTEM_PROMPT,
	SYSTEM_PROMPT_TALK,
	SYSTEM_PROMPT_TALK_WITHOUT_MCP,
	SYSTEM_PROMPT_WITHOUT_MCP,
} from "@core/prompts/system"
...
export class Task {
	...
	private async addToClineMessages(message: ClineMessage) {
		// these values allow us to reconstruct the conversation history at the time this Codee message was created
		// it's important that apiConversationHistory is initialized before we add Codee messages
	...
	private async saveClineMessagesAndUpdateHistory() {
		try {
			...
			console.error("Failed to save Codee messages:", error)
		}
	}
	...
	async ask(
		...
		// If this Codee instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of Codee now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set Codee = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
		if (this.abort) {
			throw new Error("Codee instance aborted")
		}
		let askTs: number
	...
	private async startTask(task?: string, images?: string[]): Promise<void> {
		...
		// if the extension process were killed, then on restart the clineMessages might not be empty, so we need to set it to [] when we create a new Codee client (otherwise webview would show stale messages from previous session)
		this.clineMessages = []
		this.apiConversationHistory = []
	...
	private async resumeTaskFromHistory() {
		...
		// Now present the Codee messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with Codee messages
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.abort) {
			...
			//  The way this agentic loop works is that Codee will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but Codee is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Codee responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const servers = this.mcpHub.getServers() //huqb
		const hasMCPConnected = servers.length > 0 && servers.some((s) => s.status === "connected")
		console.log("######### hasMCPConnected:", hasMCPConnected)

		let systemPrompt = ""
		if (this.chatSettings?.mode === "plan") {
			systemPrompt = hasMCPConnected
				? await SYSTEM_PROMPT_TALK(cwd, supportsBrowserUse, this.mcpHub, this.browserSettings)
				: await SYSTEM_PROMPT_TALK_WITHOUT_MCP(cwd, supportsBrowserUse, this.mcpHub, this.browserSettings)
		} else {
			systemPrompt = hasMCPConnected
				? await SYSTEM_PROMPT(cwd, supportsBrowserUse, this.mcpHub, this.browserSettings)
				: await SYSTEM_PROMPT_WITHOUT_MCP(cwd, supportsBrowserUse, this.mcpHub, this.browserSettings)
		}
		...
		//huqb
		let lastEnvironmentDetails: Anthropic.TextBlockParam | undefined
		// 处理truncatedConversationHistory中的environment_details,只保留最后一条
		for (const message of contextManagementMetadata.truncatedConversationHistory) {
			if (message.role === "user" && Array.isArray(message.content)) {
				const envDetails = message.content.find(
					(content): content is Anthropic.TextBlockParam =>
						content.type === "text" &&
						"text" in content &&
						content.text.startsWith("<environment_details>") &&
						content.text.endsWith("</environment_details>"),
				)
				if (envDetails) {
					lastEnvironmentDetails = envDetails
				}
			}
		}

		// 过滤掉所有旧的environment_details
		const filteredHistory = contextManagementMetadata.truncatedConversationHistory.map((message) => {
			if (message.role === "user" && Array.isArray(message.content)) {
				return {
					...message,
		...
		// 如果找到了environment_details,添加到最后一条user消息中
		if (lastEnvironmentDetails) {
			const lastUserMessageIndex =
				filteredHistory.length - 1 - [...filteredHistory].reverse().findIndex((m) => m.role === "user")
			if (lastUserMessageIndex !== -1) {
				const lastUserMessage = filteredHistory[lastUserMessageIndex]
				if (Array.isArray(lastUserMessage.content)) {
					lastUserMessage.content = [...lastUserMessage.content, lastEnvironmentDetails]
				} else {
					lastUserMessage.content = [{ type: "text" as const, text: lastUserMessage.content }, lastEnvironmentDetails]
				}
			}
		}

		contextManagementMetadata.truncatedConversationHistory = filteredHistory //huqb end
		// //huqb 1
		// // 在创建API请求前添加日志
		...
		// 	// 写入到本地文件
		// 	await fs.appendFile('/Users/huqb01/api_log.txt', `\n[@@@@@@@@@@@@@@ request]\n${logContent}\n\n\n[############### response]\n\n`);
		// 	// 控制台输出
		// 	// console.log('[API REQUEST]', logContent);
		// };

		// await logApiRequest(); // 调用日志记录
		// //huqb 2
				...
				switch (block.name) {
					...
					case "replace_in_file": {
						...
						try {
							...
							if (diff) {
								...
								// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but Codee throws error
							...
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, relPath)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
								...
								// Mark the file as edited by Codee to prevent false "recently modified" warnings
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to read ${path.basename(absolutePath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to view directory ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to view source code definitions in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to search files in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
							...
							if (block.partial) {
								...
								if (action === "launch") {
									...
									if (this.shouldAutoApproveTool(block.name)) {
										...
										showNotificationForApprovalIfAutoApprovalEnabled(
											`Codee wants to use a browser and launch ${url}`,
										)
										this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
						...
						try {
							if (block.partial) {
								...
								if (
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to execute a command: ${command}`,
									)
							...
							if (block.partial) {
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`Codee tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
								...
								if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to use ${tool_name} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`Codee wants to access ${uri} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
									const didApprove = await askApproval("use_mcp_server", completeMessage)
									if (!didApprove) {
					...
					case "new_task": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "Codee wants to start a new task...",
										message: `Codee is suggesting to start a new task with: ${context}`,
									})
								}
					...
					case "condense": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "Codee wants to condense the conversation...",
										message: `Codee is suggesting to condense your conversation with: ${context}`,
									})
								}
					...
					case "report_bug": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "Codee wants to create a github issue...",
										message: `Codee is suggesting to create a github issue with the title: ${title}`,
									})
								}
								...
								const clineVersion =
									vscode.extensions.getExtension("Codee.codee")?.packageJSON.version || "Unknown"
					...
					case "attempt_completion": {
						...
						if (command) {
							...
							// TODO: currently we don't handle if this command fails, it could be useful to let Codee know and retry
	...
	async recursivelyMakeClineRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		...
		if (this.consecutiveMistakeCount >= 3) {
			if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Codee is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Codee uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `Codee has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			await this.ask(
				"auto_approval_max_req_reached",
				`Codee has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
			...
			try {
				this.checkpointTracker = await pTimeout(
					CheckpointTracker.create(this.taskId, this.context.globalStorageUri.fsPath, this.enableCheckpoints),
					{
						milliseconds: 15_000,
						message:
							"Checkpoints taking too long to initialize. Consider re-opening Codee in a project that uses git, or disabling checkpoints.",
					},
				)
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : "Unknown error"
		...
		try {
			...
			try {
				for await (const chunk of stream) {
					...
					// //huqb 1
					// // 记录接收到的每个chunk
					// const logResponse = async (chunkData: any) => {
					// 	await fs.appendFile('/Users/huqb01/api_log.txt', `${logString}`);
					// 	// console.log('[API RESPONSE]', logContent);
					// };

					// await logResponse(chunk); // 调用日志记录
					// //huqb 2
				...
				// abandoned happens when extension is no longer waiting for the Codee instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.abandoned) {
			...
			// OpenRouter/Codee may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		// It could be useful for Codee to know if the user went from one or no file to another between messages, so we always include this context
		...
		const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if Codee ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
