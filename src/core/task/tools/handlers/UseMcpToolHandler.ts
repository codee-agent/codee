	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (mcp_arguments) {
			try {
				...
				config.taskState.consecutiveMistakeCount++
				await config.callbacks.say("error", `Codee tried to use ${tool_name} with an invalid JSON argument. Retrying...`)
		...
		if (config.callbacks.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
			...
			const notificationMessage = `Codee wants to use ${tool_name || "unknown tool"} on ${server_name || "unknown server"}`
