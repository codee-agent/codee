	...
	async *createMessage(systemPrompt: string, messages: ClineStorageMessage[], tools?: OpenAITool[]): ApiStream {
		try {
			...
			for await (const chunk of stream) {
				Logger.debug("CodeeHandler chunk:" + JSON.stringify(chunk))
				...
				if ((choice?.finish_reason as string) === "error") {
					...
					if (choiceWithError.error) {
						...
						throw new Error(
							"Codee Mid-Stream Error: Stream terminated with error status but no error details provided",
						)
					}
				}
			...
			if (!didOutputUsage) {
				Logger.warn("Codee API did not return usage chunk, fetching from generation endpoint")
			...
			Logger.error("Codee API Error:", error)
			throw error
		}
	}
