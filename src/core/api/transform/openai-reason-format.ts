...
import { parseAssistantMessageV2 } from "../../assistant-message/parse-assistant-message"


export function convertToOpenAiReasonMessages(
	...
	for (const anthropicMessage of anthropicMessages) {
         ...
         */
			if (anthropicMessage.role === "user") {
				...
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					nonToolMessages: (Anthropic.TextBlockParam | Anthropic.ImageBlockParam | Anthropic.ThinkingBlockParam)[]
					toolMessages: Anthropic.ToolUseBlockParam[]
				}>(
					(acc, part) => {
						if (part.type === "tool_use") {//这里是authropic转openai
							acc.toolMessages.push(part)
						} else if (part.type === "text" || part.type === "image") {
							if (part.type === "text") {
								// huqb 这里兼容cline/codee的 文字加xml工具的混合模式
								// Use V2 parser to handle XML-based tool calls in text
								const parsedBlocks = parseAssistantMessageV2(part.text)
								...
								parsedBlocks.forEach(block => {
									if (block.type === "text") {
										...
										// Convert parsed ToolUse to Anthropic.ToolUseBlockParam
										//huqb 原cline的xml中没有属性id字段，则还是走原来的业务逻辑，否则就走标准的tool calls
										if (block.toolId) {
				...
				openAiMessages.push({
					...
					...(content && content?.length > 0 && { content }),
  					...(reasoning && reasoning?.length > 0 && { reasoning_content: reasoning }),
