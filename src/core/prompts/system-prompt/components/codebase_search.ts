...
export async function getCodeBaseSearch(variant: PromptVariant, context: SystemPromptContext): Promise<string | undefined> {
	if (!context.isCodebaseSearchAvailable) {
		return undefined
	}
...
- path: (optional) Limit search to specific subdirectory (relative to the current workspace directory ${context.cwd || process.cwd}). Leave empty for entire workspace.
