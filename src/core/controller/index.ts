...
export class Controller {
	...
	async setCodeeId(id: number, userName: string) {
		this.stateManager.setSecret("codeeId", String(id))
		this.stateManager.setSecret("codeeUserName", userName)
	}

	async getCodeeToken() {
		return this.stateManager.getSecretKey("codeeToken")
	}
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			this.stateManager.setSecret("codeeId", undefined)
			this.stateManager.setSecret("codeeApiKey", undefined)
			this.stateManager.setSecret("codeeToken", undefined)
			this.stateManager.setSecret("codeeUserName", undefined)
			...
			// todo: updateAutocompleteConfig
			// const { updateAutocompleteConfig } = require("@continuedev/core/util/codaiConfigUtil")
			// updateAutocompleteConfig({
			// 	provider: VALUE_DEFAULT_AUTOCOMPLETE_PROVIDER,
			// })

			await this.postStateToWebview()
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
				message: "Successfully logged out of Codee",
			})
		} catch (_error) {
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async handleAuthCallback(customToken: string, apiKey: string | null = null) {
		try {
			...
			HostProvider.window.showMessage({
				...
				message: "Failed to log in to Codee",
			})
	...
	*/
	...
	async getCurrentWorkspaceCodeIndexManager(): Promise<CodeIndexManager | undefined> {
		return await CodeIndexManager.getInstance(this.context)
	}

	async startCodeIndexing() {
		try {
			const manager = await this.getCurrentWorkspaceCodeIndexManager()
			console.log("startIndexing111", manager, "22222", this.stateManager)
			if (!manager) {
				return
			}
			const configed = await manager.initialize(this.stateManager)
			if (!configed) return
			if (manager.isFeatureEnabled && manager.isFeatureConfigured) {
				if (!manager.isInitialized) {
					await manager.initialize(this.stateManager)
				}
