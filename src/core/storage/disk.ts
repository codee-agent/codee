...
export const GlobalFileNames = {
	...
	mcpSettings: "codee_mcp_settings.json",//huqb
	clineRules: ".codeerules",
	workflows: ".codeerules/workflows",
	hooksDir: ".codeerules/hooks",
	clineruleSkillsDir: ".codeerules/skills",
	clineSkillsDir: ".codee/skills",
 ...
 */
export function getCodeeHomePath(): string {//huqb
	return path.join(os.homedir(), ".codee")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const clineRulesDir = path.join(userDocumentsPath, "Codee", "Rules")
	try {
		await fs.mkdir(clineRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Codee", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
...
export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const clineWorkflowsDir = path.join(userDocumentsPath, "Codee", "Workflows")
	try {
		await fs.mkdir(clineWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Codee", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
...
export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Codee", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Codee", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const clineHooksDir = path.join(userDocumentsPath, "Codee", "Hooks")
	try {
		await fs.mkdir(clineHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Codee", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
...
export async function ensureSkillsDirectoryExists(): Promise<string> {
	const clineSkillsDir = path.join(getCodeeHomePath(), "skills")
	try {
