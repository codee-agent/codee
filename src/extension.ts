...
export async function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel("Codee")
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Codee version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			const lastShownPopupNotificationVersion = context.globalState.get<string>("codeeLastPopupNotificationVersion")
			...
			if (currentVersion !== lastShownPopupNotificationVersion && previousVersion) {
				...
				const message = `Codee has been updated to v${currentVersion}`
				...
				await context.globalState.update("codeeLastPopupNotificationVersion", currentVersion)
			}
			// Always update the main version tracker for the next launch.
			await context.globalState.update("codeeVersion", currentVersion)
		}
	} catch (error) {
	...
	const openClineInNewTab = async () => {
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "Codee", targetCol, {
			enableScripts: true,
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codee.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand("codee.focusChatInput") // Ensure Codee is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codee.fixWithCodee", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("codee.focusChatInput")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codee.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("codee.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
				return
		...
		vscode.commands.registerCommand("codee.focusChatInput", async () => {
			...
			if (activeWebviewProvider?.view && activeWebviewProvider.view.hasOwnProperty("reveal")) {
				...
				if (!activeWebviewProvider) {
					...
					await vscode.commands.executeCommand("codee.openInNewTab")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("codee.generateGitCommitMessage", async () => {
