...
import { Transport, McpConnection, McpTransportType, McpServerConfig, CodeIndexStateUpdate } from "./types"
import { BaseConfigSchema, ServerConfigSchema, McpSettingsSchema } from "./schemas"
import { getHostBridgeProvider } from "@/hosts/host-providers"
import { CodeIndexMcp } from "./CodeIndexMcp"
...
export class McpHub {
	...
	private CodeIndexState: CodeIndexStateUpdate = {}
  ...
  "mcpServers": {
	...
	private async handleAfterFetchTools(serverName: string, tools: McpTool[]) {
		// set project path
		if (serverName === "code-index") {
			const disabled = this.connections.find((conn) => conn.server.name === "code-index")?.server.disabled
			if (disabled) {
				this.CodeIndexState = { status: "disabled" }
				return
			}
			const workspacePath = getWorkspacePath()
			if (workspacePath) {
				if (tools.length === 0) {
					this.CodeIndexState = { status: "error" }
					return
				}
				try {
					await this.callTool("code-index", "set_project_path", { path: workspacePath })
				} catch {
					this.CodeIndexState = { status: "error" }
				}
				this.CodeIndexState = { status: "success", percent: 100 }
			} else {
				this.CodeIndexState = { status: "no_workspace", percent: 100 }
			}
		}
	}
	...
	private async fetchToolsList(serverName: string): Promise<McpTool[]> {
		try {
			...
			this.handleAfterFetchTools(serverName, tools)
			return tools
		} catch (error) {
			console.error(`Failed to fetch tools for ${serverName}:`, error)
			this.handleAfterFetchTools(serverName, [])
			return []
		}
	}
	...
	// wy
	public async writeServeToFile(serverName: string, serverConfig: any, forceWrite: boolean = false) {
		const config = await this.readAndValidateMcpSettingsFile()
		if (!config) {
			throw new Error("Failed to read or validate MCP settings")
		}
		if (config.mcpServers[serverName] && !forceWrite) {
			console.log("write config.mcpServers[serverName] exists =============", serverName)
			const serverOrder = Object.keys(config.mcpServers || {})
			return this.getSortedMcpServers(serverOrder)
		}
	...
	private async configCodeIndex() {
		const codeIndex = new CodeIndexMcp()
		const config = await codeIndex.download()
		if (config.status) {
			this.CodeIndexState.status = config.status
			return
		}
		this.CodeIndexState = { status: "indexing", percent: 30 }
		await this.writeServeToFile("code-index", config)

		const workspacePath = getWorkspacePath()
		if (!workspacePath) {
			this.CodeIndexState = { status: "no_workspace" }
			return
		}
		...
		let count = 0
		const server = this.connections.find((conn) => conn.server.name === "code-index")?.server
		if (!server) {
			this.CodeIndexState = { status: "error" }
			return
		}
		...
		let serverTools = server?.tools?.length
		while (!serverTools || serverTools === 0) {
			if (count >= 30) {
				// timeout 60s
				console.log("configCodeIndex timeout: ")
				this.CodeIndexState = { status: "error" }
				return
			}
			await wait(2000)
			serverTools = this.connections.find((conn) => conn.server.name === "code-index")?.server.tools?.length
			count++
		}
		this.CodeIndexState = { status: "indexing", percent: 60 }
		try {
			const result = await this.callTool("code-index", "check_project_path", { path: workspacePath })
			console.log("result===================", result.content)
			let content = result.content.filter((res) => res.type === "text")
			if (content.length && content[0].text) {
				let exists = JSON.parse(content[0].text).exists
				if (!exists) {
					await this.callTool("code-index", "set_project_path", { path: workspacePath })
				}
			}
		} catch (err) {
			console.log("configCodeIndex err: ", err)
			this.CodeIndexState = { status: "error" }
			return
		}
		this.CodeIndexState = { status: "success", percent: 100 }
	...
	public get currentCodeIndexState(): CodeIndexStateUpdate {
