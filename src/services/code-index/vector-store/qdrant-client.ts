...
export class QdrantVectorStore implements IVectorStore {
	...
	/**
	 * Initializes the vector store with retry mechanism
	...
	async initialize(): Promise<boolean> {
		...
		while (attempts < maxAttempts) {
			try {
				console.log(`[QdrantVectorStore] Initialization attempt ${attempts + 1}/${maxAttempts}`);
				return await this._initializeInternal();
			} catch (error: any) {
				attempts++;
				
				if (attempts >= maxAttempts) {
					console.error(`[QdrantVectorStore] Initialization failed after ${maxAttempts} attempts`);
				...
				console.log(`[QdrantVectorStore] Initialization attempt ${attempts} failed, retrying in ${retryDelay}ms...`);
				console.log(`[QdrantVectorStore] Error:`, error?.message || error);
		...
		throw new Error("Unexpected error in initialize method");
	}

	/**
	 * Internal initialization method without retry logic
	 * @returns Promise resolving to boolean indicating if a new collection was created
	 */
	...
	private async _recreateCollectionWithNewDimension(existingVectorSize: number): Promise<boolean> {
		...
		try {
			...
			const dimensionMismatchError = new Error("vectorDimensionMismatch")
