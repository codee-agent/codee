...
import { DEFAULT_SEARCH_MIN_SCORE, DEFAULT_MAX_SEARCH_RESULTS, DEFAULT_CODEBASE_INDEX_CONFIG } from "./constants/index"
...
export class CodeIndexConfigManager {
	...
	constructor(private readonly cacheService: StateManager) {
	 ...
	 */
	public getCacheService(): StateManager {
	...
	private _loadAndSetConfiguration(): void {
		...
		let codebaseIndexConfig = this.cacheService?.getGlobalStateKey("codebaseIndexConfig")
		if (!codebaseIndexConfig || Object.keys(codebaseIndexConfig).length === 0) {
			codebaseIndexConfig = DEFAULT_CODEBASE_INDEX_CONFIG
			this.cacheService?.setGlobalState('codebaseIndexConfig', DEFAULT_CODEBASE_INDEX_CONFIG)
		}
		const {
		...
		console.log('codebaseIndexEmbedderProvider, ', codebaseIndexEmbedderProvider)

		const openAiKey = this.cacheService?.getSecretKey("codeIndexOpenAiKey") ?? ""
		const qdrantApiKey = this.cacheService?.getSecretKey("codeIndexQdrantApiKey") ?? ""
		// Fix: Read OpenAI Compatible settings from the correct location within codebaseIndexConfig
		const openAiCompatibleBaseUrl = codebaseIndexConfig.codebaseIndexEmbedderBaseUrl ?? DEFAULT_CODEBASE_INDEX_CONFIG.codebaseIndexEmbedderBaseUrl
		const openAiCompatibleApiKey = (openAiKey.length > 0 ? openAiKey : this.cacheService?.getSecretKey("codeeApiKey")) ?? ""//huqb
		...
		this.geminiOptions = openAiKey.length > 0 ? { apiKey: openAiKey } : undefined
		this.mistralOptions = openAiKey.length > 0 ? { apiKey: openAiKey } : undefined
		this.vercelAiGatewayOptions = openAiKey.length > 0 ? { apiKey: openAiKey } : undefined
	}
	...
	public async loadConfiguration(): Promise<{
		...
		// await this.cacheService.refreshSecrets()
