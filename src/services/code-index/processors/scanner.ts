...
export class DirectoryScanner implements IDirectoryScanner {
	...
	public async scanDirectory(
		...
		const scanWorkspace = await getWorkspacePathForContext(directoryPath)
		...
		// Initialize ClineIgnoreController if not provided
		const ignoreController = new ClineIgnoreController(directoryPath)

		await ignoreController.initialize()

		// Filter paths using .clineignore
					...
					if (onError) {
						onError(
							error instanceof Error
								...
								: new Error("unknownErrorProcessingFile"),
						)
					}
				}
			}),
						...
						if (onError) {
							...
							onError(
								error instanceof Error
									...
									: new Error("unknownErrorDeletingPoints"),
							)
						}
	...
	private async processBatch(
		...
		if (!success && lastError) {
			...
			if (onError) {
				...
				onError(new Error("failedToProcessBatchWithError"),)
			}
		}
	}
}
