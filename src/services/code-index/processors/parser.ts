		...
		while (queue.length > 0) {
			...
			if (currentNode.text.length >= MIN_BLOCK_CHARS) {
				...
				if (currentNode.text.length > MAX_BLOCK_CHARS * MAX_CHARS_TOLERANCE_FACTOR) {
					if (currentNode.children.filter((child: any) => child !== null).length > 0) {
						// If it has children, process them instead
						queue.push(...currentNode.children.filter((child: any) => child !== null))
					} else {
					...
					const identifier =
						...
						currentNode.children.find((c: any) => c?.type === "identifier")?.text ||
						null
