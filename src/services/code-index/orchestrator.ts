...
export class CodeIndexOrchestrator {
	...
	public async startIndexing(): Promise<void> {
		...
		if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length === 0) {
			this.stateManager.setSystemState("Error", "indexingRequiresWorkspace")
		...
		try {
			...
			if (cumulativeBlocksIndexed === 0 && cumulativeBlocksFoundSoFar > 0) {
				if (batchErrors.length > 0) {
					...
					throw new Error("indexingFailedNoBlocks")
				}
			}
			...
			if (cumulativeBlocksFoundSoFar > 0 && cumulativeBlocksIndexed === 0) {
				throw new Error("indexingFailedCritical")
			}

			await this._startWatcher()

			this.stateManager.setSystemState("Indexed", 'fileWatcherStarted')
		} catch (error: any) {
	...
	public stopWatcher(): void {
		...
		if (this.stateManager.state !== "Error") {
			this.stateManager.setSystemState("Standby", "fileWatcherStopped")
		}
		this._isProcessing = false
	}
