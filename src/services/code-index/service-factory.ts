...
export class CodeIndexServiceFactory {
	...
	public createEmbedder(): IEmbedder {
		...
		if (provider === "openai") {
			...
			if (!apiKey) {
				throw new Error("openAiConfigMissing")
			}
			...
			if (!config.ollamaOptions?.ollamaBaseUrl) {
				throw new Error("ollamaConfigMissing")
			}
			...
			if (!config.openAiCompatibleOptions?.baseUrl || !config.openAiCompatibleOptions?.apiKey) {
				throw new Error("openAiCompatibleConfigMissing")
			}
			...
			if (!config.vercelAiGatewayOptions?.apiKey) {
				throw new Error("vercelAiGatewayConfigMissing")
			}
	...
	public createVectorStore(): IVectorStore {
		...
		if (vectorSize === undefined || vectorSize <= 0) {
			if (provider === "openai-compatible") {
				throw new Error("vectorDimensionNotDeterminedOpenAiCompatible")
			} else {
				throw new Error("vectorDimensionNotDetermined")
			}
		}

		if (!config.qdrantUrl) {
			throw new Error("qdrantUrlMissing")
		}
	...
	public createServices(
		...
		if (!this.configManager.isFeatureConfigured) {
			throw new Error("codeIndexingNotConfigured")
		}
