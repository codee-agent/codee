...
export function handleValidationError(
	...
	if (errorMessage) {
		if (
			...
			return { valid: false, error: "connectionFailed" }
		}

		if (errorMessage.includes("Failed to parse response JSON")) {
			return { valid: false, error: "invalidResponse" }
		}
	}
	...
	return { valid: false, error: "configurationError" }
}
...
export function formatEmbeddingError(error: any, maxRetries: number): Error {
	...
	if (statusCode === 401) {
		return new Error("authenticationFailed")
	} else if (statusCode) {
		return new Error("failedWithStatus")
	} else {
