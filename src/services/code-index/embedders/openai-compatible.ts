...
export class OpenAICompatibleEmbedder implements IEmbedder {
	...
	constructor(baseUrl: string, apiKey: string, modelId?: string, maxItemTokens?: number) {
		if (!baseUrl) {
			throw new Error("embeddings:validation.baseUrlRequired")
		}
		if (!apiKey) {
			throw new Error("embeddings:validation.apiKeyRequired")
		}
		...
		const processedTexts = queryPrefix
			? texts.map((text, index) => {
					...
					if (estimatedTokens > MAX_ITEM_TOKENS) {
						console.warn(
							"embeddings:textWithPrefixExceedsTokenLimit"
						)
	...
	private async _embedBatchWithRetries(
		...
		throw new Error("embeddings:failedMaxAttempts")
	}
