...
import { OpenAiNativeHandler } from "@/core/api/providers/openai-native"
...
export class OpenAiEmbedder extends OpenAiNativeHandler implements IEmbedder {
		...
		const processedTexts = queryPrefix
			? texts.map((text, index) => {
					...
					if (estimatedTokens > MAX_ITEM_TOKENS) {
						console.warn(
							"textWithPrefixExceedsTokenLimit"
						)
		...
		while (remainingTexts.length > 0) {
			...
			for (let i = 0; i < remainingTexts.length; i++) {
				...
				if (itemTokens > MAX_ITEM_TOKENS) {
					console.warn("textExceedsTokenLimit")
	...
	private async _embedBatchWithRetries(
		...
		throw new Error("failedMaxAttempts")
	}
