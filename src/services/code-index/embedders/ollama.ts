...
export class CodeIndexOllamaEmbedder implements IEmbedder {
	...
	async createEmbeddings(texts: string[], model?: string): Promise<EmbeddingResponse> {
		...
		const processedTexts = queryPrefix
			? texts.map((text, index) => {
					...
					if (estimatedTokens > MAX_ITEM_TOKENS) {
						console.warn("textWithPrefixExceedsTokenLimit")
		...
		try {
			...
			if (!response.ok) {
				let errorBody = "couldNotReadErrorBody"
				try {
			...
			if (!embeddings || !Array.isArray(embeddings)) {
				throw new Error("ollama.invalidResponseStructure")
			}
			...
			if (error.name === "AbortError") {
				throw new Error("embeddings:validation.connectionFailed")
			} else if (error.message?.includes("fetch failed") || error.code === "ECONNREFUSED") {
				throw new Error("embeddings:ollama.serviceNotRunning")
			} else if (error.code === "ENOTFOUND") {
				throw new Error("embeddings:ollama.hostNotFound")
			}

			// Re-throw a more specific error for the caller
			throw new Error("embeddings:ollama.embeddingFailed")
		}
	}
			...
			async () => {
				...
				if (!modelsResponse.ok) {
					if (modelsResponse.status === 404) {
						return {
							valid: false,
							error: "embeddings:ollama.serviceNotRunning",
						}
					}
					return {
						valid: false,
						error: "embeddings:ollama.serviceUnavailable"
					}
				}
				...
				if (!modelExists) {
					...
					return {
						valid: false,
						error: "embeddings:ollama.modelNotFound",
					}
				}
				...
				if (!testResponse.ok) {
					return {
						valid: false,
						error: "embeddings:ollama.modelNotEmbeddingCapable",
					}
				}
			...
			"ollama",
			{
				beforeStandardHandling: (error: any) => {
					...
					if (
						...
						return {
							valid: false,
							error: "embeddings:ollama.hostNotFound",
						}
