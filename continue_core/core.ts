...
import { setupBestConfig, setupLocalConfig, setupQuickstartConfig } from "./config/onboarding"
...
import { DiffLine, type ContextItemId, type IDE, type IndexingProgressUpdate } from "."
...
export class Core {
	...
	private readonly indexingPauseToken = new PauseToken(this.globalContext.get("indexingPaused") === true)
	...
	constructor(
		...
		this.configHandler = new ConfigHandler(this.ide, ideSettingsPromise, this.llmLogger, sessionInfoPromise)
		...
		this.configHandler.onConfigUpdate(async (result) => {
			...
			this.messenger.send("configUpdate", {
				...
				usingContinueForTeams: (await ideSettingsPromise).enableControlServerBeta,
			})
		...
		this.codebaseIndexerPromise = new Promise(async (resolve) => (codebaseIndexerResolve = resolve))

		let continueServerClientResolve: (_: any) => void | undefined
		this.continueServerClientPromise = new Promise((resolve) => (continueServerClientResolve = resolve))
		...
		void ideSettingsPromise.then((ideSettings) => {
			...
			codebaseIndexerResolve(
				new CodebaseIndexer(this.configHandler, this.ide, this.indexingPauseToken, continueServerClient),
			)
		...
		on("context/getContextItems", async (msg) => {
			...
			const { name, query, fullInput, selectedCode, selectedModelTitle } = msg.data

			const llm = await this.configHandler.llmFromTitle(selectedModelTitle)
			const provider =
				config.contextProviders?.find((provider) => provider.description.title === name) ??
				[
		...
		on("llm/streamChat", (msg) => llmStreamChat(this.configHandler, this.abortedMessageIds, msg, ide, this.messenger))

		on("llm/complete", async (msg) => {
			const model = await this.configHandler.llmFromTitle(msg.data.title)
			const completion = await model.complete(msg.data.prompt, new AbortController().signal, msg.data.completionOptions)
			return completion
		})
		...
		on("chatDescriber/describe", async (msg) => {
			const currentModel = await this.configHandler.llmFromTitle(msg.data.selectedModelTitle)
		...
		on("streamDiffLines", (msg) => streamDiffLinesGenerator(this.configHandler, this.abortedMessageIds, msg))
		...
		on("index/indexingProgressBarInitialized", async (msg) => {
			...
			if (this.codebaseIndexingState.status !== "loading") {
				void this.messenger.request("indexProgress", this.codebaseIndexingState)
			}
		})
			...
			if (data?.uris?.length) {
				...
				for (const uri of data.uris) {
					...
					if (uri.endsWith(".continuerc.json") || uri.endsWith(".prompt") || uri.endsWith(SYSTEM_PROMPT_DOT_FILE)) {
						await this.configHandler.reloadConfig()
					} else if (uri.endsWith(".codeeignore") || uri.endsWith(".gitignore")) {
		...
		on("didChangeSelectedOrg", async (msg) => {
			await this.configHandler.setSelectedOrgId(msg.data.id, msg.data.profileId)
		})
		...
		on("didChangeActiveTextEditor", async ({ data: { filepath } }) => {
			try {
				...
				console.error(`didChangeActiveTextEditor: failed to update recentlyEditedFiles cache for ${filepath}`)
			}
		})
		...
		on("tools/call", async ({ data: { toolCall, selectedModelTitle } }) => {
			...
			const contextItems = await callTool(tool, JSON.parse(toolCall.function.arguments || "{}"), {
				ide: this.ide,
				llm,
				fetch: (url, init) => fetchwithRequestOptions(url, init, config.requestOptions),
				tool,
	...
	private async refreshCodebaseIndexFiles(files: string[]) {
		...
		if (this.indexingCancellationController && !this.indexingCancellationController.signal.aborted) {
			return
		}
		this.indexingCancellationController = new AbortController()
		for await (const update of (await this.codebaseIndexerPromise).refreshFiles(files)) {
			let updateToSend = { ...update }
			if (update.status === "failed") {
				updateToSend.status = "done"
				updateToSend.desc = "Indexing complete"
