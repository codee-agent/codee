...
const statusBarItemText = (status: StatusBarStatus | undefined, loading?: boolean, error?: boolean) => {
	if (error) {
		return "$(alert) Codee (FATAL ERROR)"
	}

	switch (status) {
		case undefined:
			if (loading) {
				return "$(loading~spin) Codee"
			}
			break
		case StatusBarStatus.Disabled:
			return "$(circle-slash) Codee"
		case StatusBarStatus.Enabled:
			return "$(check) Codee"
		case StatusBarStatus.Paused:
			return "$(debug-pause) Codee"
	}
	return ""
}
...
export function setupStatusBar(status: StatusBarStatus | undefined, loading?: boolean, error?: boolean) {
	...
	const text = statusBarItemText(status, loading, statusBarError)
	if (text) {
		statusBarItem.text = text
	}
	statusBarItem.tooltip = statusBarItemTooltip(status ?? statusBarStatus)
	statusBarItem.command = "codee.openTabAutocompleteConfigMenu" //huqb
