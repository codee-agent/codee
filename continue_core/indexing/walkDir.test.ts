...
describe("walkDir functions", () => {
	...
	describe("walkDir", () => {
		...
		it("should respect .gitignore", async () => {
			...
			expect(files).toEqual(expect.not.arrayContaining([expect.stringContaining("node_modules/pkg/file.js")]))
			expect(files.sort()).toEqual(
				[expect.stringContaining("src/file1.ts"), expect.stringContaining("src/file2.ts")].sort(),
			)
		})

		it("should handle include option", async () => {
			addToTestDir(["src/", ["src/file1.ts", "content1"], "src/nested/", ["src/nested/file2.ts", "content2"]])

			const dirs = await walkDir((await testIde.getWorkspaceDirs())[0], testIde, {
				include: "dirs",
			})

			expect(dirs.sort()).toEqual([expect.stringContaining("src"), expect.stringContaining("src/nested")].sort())
		})
	})

	describe("walkDirAsync", () => {
		it("should yield files asynchronously", async () => {
			addToTestDir(["src/", ["src/file1.ts", "content1"], ["src/file2.ts", "content2"]])
