...
describe("shouldIgnore", () => {
	...
	test("should return true if a folder is ignored by .codeeignore", async () => {
		addToTestDir([
			["ignored-folder/file.txt", "content"],
			[".codeeignore", "ignored-folder/"],
		])
	...
	test("should return false if a file is not ignored", async () => {
		...
		const result = await shouldIgnore(TEST_DIR + "/kept-file.txt", testIde, [TEST_DIR])
	...
	test("should return true if a .codeeignore override ignores file", async () => {
		addToTestDir([
			["override-file.txt", "content"],
			[".gitignore", "override-file.txt"],
			[".codeeignore", "!override-file.txt"],
		])
	...
	test("should return true for deeply nested file ignored by ignore files at multiple levels", async () => {
		addToTestDir([
			...
			["level1/level2/.codeeignore", "level3/"],
		])
	...
	test("should respect default file and folder ignores top level", async () => {
		addToTestDir([[".env", "contents"], ["go.sum", "contents"], ".idea/", [".idea/test.xml", "contents"]])
		...
		result = await shouldIgnore(TEST_DIR + "/.idea/test.xml", testIde, [TEST_DIR])
		expect(result).toBe(true)
	})

	test("should respect default file and folder ignores at nested level", async () => {
		addToTestDir(["nested/.idea/test.xml", ["nested/.env", "contents"], ["nested/go.sum", "contents"]])

		let result = await shouldIgnore(TEST_DIR + "/nested/.idea", testIde, [TEST_DIR])
		expect(result).toBe(true)

		result = await shouldIgnore(TEST_DIR + "/nested/.idea/test.xml", testIde, [TEST_DIR])
		expect(result).toBe(true)

		result = await shouldIgnore(TEST_DIR + "/nested/.env", testIde, [TEST_DIR])
		expect(result).toBe(true)

		result = await shouldIgnore(TEST_DIR + "/nested/go.sum", testIde, [TEST_DIR])
		expect(result).toBe(true)
	})
})
