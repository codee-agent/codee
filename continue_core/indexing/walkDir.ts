...
export async function getIgnoreContext(
	...
	// These are done separately so that .codeeignore can override .gitignore
	const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore")
	const continueIgnoreFile = dirFiles.find((name) => name === ".codeeignore")
	...
	const getContinueIgnorePatterns = async () => {
		if (continueIgnoreFile) {
			const contents = await ide.readFile(`${currentDir}/.codeeignore`)
			return gitIgArrayFromFile(contents)
		}
		return []
	}
	...
	const ignoreContext = ignore()
		...
		.add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .codeeignore - this is combined for speed
		.add(ignoreArrays[1]) // local .codeeignore
