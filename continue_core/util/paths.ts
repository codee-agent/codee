...
const CONTINUE_GLOBAL_DIR = process.env.CONTINUE_GLOBAL_DIR ?? path.join(os.homedir(), ".codee")
...
export function getGlobalContinueIgnorePath(): string {
	const continueIgnorePath = path.join(getContinueGlobalPath(), ".codeeignore")
...
export function getPathToRemoteConfig(remoteConfigServerUrl: string): string {
	...
	try {
		url =
			typeof remoteConfigServerUrl !== "string" || remoteConfigServerUrl === "" ? undefined : new URL(remoteConfigServerUrl)
	} catch (e) {}
...
export function readAllGlobalPromptFiles(folderPath: string = getGlobalPromptsPath()): { path: string; content: string }[] {
