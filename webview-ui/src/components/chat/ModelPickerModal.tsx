...
import { Brain, Check, ChevronDownIcon, Search, Settings } from "lucide-react"
...
const ModelPickerModal: React.FC<ModelPickerModalProps> = ({ isOpen, onOpenChange, currentMode, children }) => {
	...
	const { handleModeFieldChange, handleModeFieldsChange, handleFieldsChange, handleFieldChange } = useApiConfigurationHandlers()
	...
	const allModels = useMemo((): ModelItem[] => {
		...
		if (selectedProvider === "codee") {
			if (codeeModels?.length === 0) {
	...
	const handleSelectModel = useCallback(
		(modelId: string, modelInfo?: ModelInfoType) => {
			...
			} else if (selectedProvider === "codee") {
				handleFieldChange("codeeModelId", modelId)
			} 
			else {
