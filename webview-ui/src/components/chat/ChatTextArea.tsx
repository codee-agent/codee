...
import { EnhancePromptsRequest } from "@shared/proto/index.cline"
...
import { FileServiceClient, ModelsServiceClient, StateServiceClient, BusinessServiceClient } from "@/services/grpc-client"
import {
...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const modelDisplayName = useMemo(() => {
			...
			console.log('modelDisplayName', selectedProvider, 'id', selectedModelId)
			const {
		...
		const handleEnhancePrompt = useCallback(() => {
			...
			setIsEnhancing(true)
			BusinessServiceClient.enhancePrompts(
				EnhancePromptsRequest.create({
					mode: mode,
				...
				if (response.value) {
					...
					const cleanedText = response.value.replace(/<think>.*?<\/think>|<thinking>.*?<\/thinking>/gis, "")
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<Tooltip>
						<TooltipContent
							...
							side="top">
							{`In ${shownTooltipMode === "act" ? "Act" : "Plan"}  mode, Codee will ${shownTooltipMode === "act" ? "complete the task immediately" : "gather information to architect a plan"}`}
