...
export function normalizeApiConfiguration(
	...
	switch (provider) {
		...
		case "codee":
			const codeeModelId = apiConfiguration?.codeeModelId
			let codeeModeInfo = apiConfiguration?.planModeOpenAiModelInfo || openAiModelInfoSaneDefaults
			if (codeeModelId === "deepseek-v3") {
				//huqb
				codeeModeInfo = {
					...codeeModeInfo,
					contextWindow: 64000,
			...
			return {
				selectedProvider: provider,
				selectedModelId: codeeModelId || "",
				selectedModelInfo: codeeModeInfo,
			}
		case "bedrock":
